var clientId = ([1e7] + -1e3 + -4e3 + -8e3 + -1e11).replace(/[018]/g, c =>
  (
    c ^
  ...
  if (!gyroscope && alive) {
    if (cursors.left.isDown && !cursors.right.isDown) {
      ...
      if (connectedCloud) {
        ...
        if (
          beforePlayerData.isRunning !== newPlayerData.isRunning ||
          beforePlayerData.direction !== newPlayerData.direction
        ) {
          let [x, y] = getNormalizedPositions(player.body.x, player.body.y);
          client.publish(
            lobbyId,
    ...
    if (!player.body.touching.down) {
      ...
      if (player.body.velocity.x === 0) {
        ...
        if (player.body.velocity.x > 0)
          player.anims.play('rightJump' + avatars.indexOf(avatar));
        if (player.body.velocity.x < 0)
          player.anims.play('leftJump' + avatars.indexOf(avatar));
      }
    }
    ...
    if (new Date().getTime() - lastTimeSpawn > random) {
      ...
      if (Math.random() > 0.2) {
        ...
        if (Math.random() <= 0.8) {
          ...
          let x =
            Math.random() *
              (((width - boundingWidth * 0.85) / 2 + boundingWidth * 0.85) *
                icicleConfig.maxSpawnOffset -
                ((width - boundingWidth * 0.85) / 2) *
                  icicleConfig.minSpawnOffset) +
            ((width - boundingWidth * 0.85) / 2) * icicleConfig.minSpawnOffset;

          ice = this.physics.add.sprite(
            x,
            -1 * (boundingHeight * 0.4),
            'icicle'
          );
          ...
          let penguin = this.physics.add.sprite(
            x,
            height - height * 0.2,
            'penguin'
          );
    ...
    try {
      if (otherPlayerData.isRunning && otherPlayerData.direction == -1) {
        ...
        otherPlayer.anims.play(
          'left' + avatars.indexOf(otherPlayerData.avatar)
        );
        ...
        otherPlayer.body.velocity.x = boundingWidth * 0.3;
        otherPlayer.anims.play(
          'right' + avatars.indexOf(otherPlayerData.avatar)
        );
        if (otherPlayerData.avatar.crop) {
          otherPlayer.setCrop(0, 72.248, player.width, 286.752);
          otherPlayer.height = 286.752;
        ...
        otherPlayer.anims.play(
          'turn' + avatars.indexOf(otherPlayerData.avatar)
        );
        if (otherPlayerData.avatar.crop) {
          otherPlayer.setCrop(0, 72.248, player.width, 286.752);
          otherPlayer.height = 286.752;
      ...
      if (!otherPlayer.body.touching.down) {
        if (otherPlayer.body.velocity.x === 0)
          otherPlayer.anims.play(
            'turnJump' + avatars.indexOf(otherPlayerData.avatar)
          );
        if (otherPlayer.body.velocity.x > 0)
          otherPlayer.anims.play(
            'rightJump' + avatars.indexOf(otherPlayerData.avatar)
          );
        if (otherPlayer.body.velocity.x < 0)
          otherPlayer.anims.play(
            'leftJump' + avatars.indexOf(otherPlayerData.avatar)
          );
        if (otherPlayerData.avatar.crop) {
  ...
  client.on('message', function(topic, message) {
    ...
    if (data.status != undefined && data.status === 'connectionRequest') {
      ...
      if (!multiplayer) {
        ...
        console.warn(
          `Spawning player: ${data.clientId} / With skin ${otherPlayerData.avatar.key}`
        );

        otherPlayer = gameObj.physics.add.sprite(
          width / 2,
          height - height * 0.5,
          otherPlayerData.avatar.key
        );
        // player.displayWidth = ;
    ...
    if (data.status != undefined && data.status === 'movement') {
      if (data.isRunning != undefined)
        otherPlayerData.isRunning = data.isRunning;
      if (data.direction != undefined)
        otherPlayerData.direction = data.direction;
      if (data.isJumping != undefined)
        otherPlayerData.isJumping = data.isJumping;
      ...
      if (data.x != undefined || data.y != undefined) {
        ...
        otherPlayer.setPosition(
          x + otherPlayer.body.width / 2,
          otherPlayer.body.y + otherPlayer.body.height / 2
        );
      }
    }
  });
...
const resize = () => {
  ...
  if (
    newWidth === width &&
    newHeight === height &&
    newBoundingWidth === boundingWidth &&
